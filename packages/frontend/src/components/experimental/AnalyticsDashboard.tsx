    ...
    <div className="flex flex-col h-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-xl shadow-2xl overflow-hidden border border-slate-700/50">
      ...
      <div className="flex-1 overflow-y-auto p-3 sm:p-6 space-y-4 sm:space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
          {/* By Type */}
          <div className="bg-slate-800/60 backdrop-blur-sm border border-slate-700/50 rounded-xl p-4 sm:p-6">
                    ...
                    >
                      ...
                      <div className="w-full h-1 bg-slate-700/50 rounded-full overflow-hidden ml-4">
                        <motion.div
                          ...
                          animate={{
                            width: `${
                              (Number(count) / (stats.total_memories || 1)) *
                              100
                            }%`,
                          }}
              ...
              <table className="min-w-full">
                <thead>
                  <tr className="border-b border-slate-700/50">
                    <th className="px-3 sm:px-4 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-medium text-slate-400 uppercase tracking-wider">
                      Model
                    </th>
                    <th className="px-3 sm:px-4 py-2 sm:py-3 text-left text-[10px] sm:text-xs font-medium text-slate-400 uppercase tracking-wider">
                      Tokens
                ...
                <tbody className="divide-y divide-slate-700/30">
                  {Object.entries(usage.by_model).map(([model, data], idx) => (
                    ...
                    >
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm font-medium text-white break-words">
                        {model}
                      </td>
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm text-slate-300">
                        {data.tokens.toLocaleString()}
                      </td>
                      <td className="px-3 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm text-green-400 font-mono">
                        ${data.cost.toFixed(4)}
                      </td>
                    </motion.tr>
                  ))}
...
function StatCard({
  ...
  return (
    <motion.div
      ...
      className="bg-slate-800/60 backdrop-blur-sm border border-slate-700/50 rounded-xl p-4 sm:p-5 hover:border-purple-500/30 hover:shadow-lg transition-all"
    >
      <div className="flex items-center justify-between mb-2 sm:mb-3">
        <p className="text-xs sm:text-sm font-medium text-slate-400">{title}</p>
        <div
          className={`p-1.5 sm:p-2 bg-gradient-to-br ${colorClasses[color]} rounded-lg`}
        >
