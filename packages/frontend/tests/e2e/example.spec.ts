...
test.describe('Companion Chat', () => {
  test('should send message and receive AI response', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="companion-message"]').last()).toContainText(/\w+/);
  });
});

test.describe('Quest Management', () => {
  test('should create quest with missions', async ({ page, userFactory, questFactory }) => {
    ...
    await page.fill('[data-testid="quest-description"]', 'Master TypeScript fundamentals');
    ...
    await page.goto('/quests');
    await expect(page.locator(`[data-testid="quest-${quest.id}"]`)).toBeVisible();
  });
});
...
test.describe('Evidence Upload', () => {
  test.skip('should upload evidence for quest', async ({ page }) => {
    ...
    // Verify upload
    await expect(page.locator('[data-testid="evidence-success"]')).toBeVisible();
  });
});
