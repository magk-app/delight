---
title: "Codex Development Environment Setup"
description: "Configure a Codex development environment for the Delight monorepo"
icon: "gear"
---
...
<Info>
One environment = One workspace that contains the entire repository. The monorepo structure (`packages/frontend/` and `packages/backend/`) lives within that single environment.
</Info>
...
<Step title="CLERK_WEBHOOK_SECRET">
Your Clerk webhook signing secret (starts with `whsec_`)
</Step>

<Step title="DATABASE_URL">
Your database connection string (if using cloud-dev mode)
</Step>

<Step title="OPENAI_API_KEY">
Your OpenAI API key (starts with `sk-`)
</Step>

<Step title="REDIS_URL">
Your Redis connection URL (if using cloud Redis)
</Step>
...
<Note>
Check if Codex supports Docker before using local mode.
</Note>
...
- **Full Setup Guide:** See [BMAD Developer Guide](./bmad-guide.mdx) for detailed local setup
- **Architecture:** See [Architecture](../architecture) for system design
- **Quick Reference:** See [Quick Reference](./quick-reference.mdx) for commands
