...
This document captures the key architectural decisions made for Delight, including rationale, alternatives considered, and consequences.
...
- **Excellent integration:** LangChain has first-class pgvector support (langchain-postgres)
- **Reduced complexity:** One database instead of two
...
- Pinecone (managed vector DB, additional cost)

**Consequences:**
- Need PostgreSQL 15+ with pgvector extension installed
- **Solution:** Use Supabase (managed PostgreSQL with pgvector pre-installed)
...
- **Migration path:** Can migrate to JSONB later if performance becomes an issue
...
**Decision:** Use GPT-4o-mini for routine AI interactions (chat, personas, quest generation), reserve GPT-4o for premium narrative only.

**Rationale:**
- **Cost:** GPT-4o-mini is ~80% cheaper than GPT-4o ($0.15/$0.60 vs $2.50/$10 per 1M tokens)
- **Quality:** Sufficient for conversational AI, persona-based responses
- **Speed:** Faster response times for better UX
- **Economics:** Enables $0.50/user/day cost target (actual: ~$0.03/user/day with GPT-4o-mini)

**Pricing Comparison:**
```
Daily user activity: ~100 messages/day
Avg message: 500 tokens (input) + 200 tokens (output)

GPT-4o cost: 100 × (500×$2.50 + 200×$10) / 1M = $0.33/day
GPT-4o-mini cost: 100 × (500×$0.15 + 200×$0.60) / 1M = $0.02/day

Savings: $0.31/day per user
```

**Alternatives Considered:**
- GPT-4o everywhere (better quality, 16x more expensive)
- Claude 3.5 Sonnet (good alternative, similar pricing to GPT-4o)
- Open source models (free after hosting, significant quality gap)

**Consequences:**
- Need to carefully design prompts for GPT-4o-mini capabilities
- Reserve GPT-4o for narrative generation where quality matters most
- Monitor quality metrics to ensure user satisfaction

---

## ADR-010: Supabase for Managed PostgreSQL

**Decision:** Use Supabase as the managed PostgreSQL provider.

**Rationale:**
- **pgvector pre-installed:** No manual extension setup
- **Zero local setup:** Developers can connect immediately
- **Free tier sufficient:** 500MB DB, 2GB data transfer (adequate for MVP)
- **Automatic backups:** Point-in-time recovery included
- **Production-ready:** Easy scaling path from free tier to production

**Alternatives Considered:**
- Local PostgreSQL (more setup, requires Docker/manual install)
- AWS RDS (more expensive, requires AWS account setup)
- Railway (good alternative, similar pricing)

**Consequences:**
- Need to configure environment variables (DATABASE_URL)
- Slightly more latency than localhost (acceptable for development)
- Production-ready from day one
- Simple migration to self-hosted if needed later
...
|-----|----------|----------------|-----------|
| 001 | Next.js 15 + React 19 | Ecosystem & streaming | Bundle size |
| 002 | FastAPI | Async-first | Manual setup |
| 003 | LangGraph + LangChain | Stateful agents | Learning curve |
| 004 | PostgreSQL pgvector | Unified storage | Requires PG 15+ |
| 005 | Hybrid SSE/WS/HTTP | Real-time UX | Connection mgmt |
| 006 | JSON (not JSONB) | Simplicity | Query performance |
| 007 | Clerk | Time to market | Vendor lock-in |
| 008 | Open source emotion | Privacy & cost | Hosting required |
| 009 | GPT-4o-mini primary | 80% cost savings | Quality trade-off |
| 010 | Supabase | Zero setup | External dependency |
