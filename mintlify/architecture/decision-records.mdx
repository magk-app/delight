...
description: Key architectural decisions made for the Delight platform, including rationale, alternatives considered, and consequences.
---

This document captures significant architectural decisions and their justifications, following the Architecture Decision Record (ADR) pattern.
...
<Warning>
Cannot create GIN indexes on JSON columns (only JSONB supports this). If performance becomes an issue with large queries, consider migrating to JSONB.
</Warning>
...
**Negative:**
- Vendor lock-in (mitigated by standardized OAuth flows)
...
**Negative:**
- Requires hosting infrastructure (CPU/GPU)
- Model loading time on startup

**Implementation:**
Use Hugging Face Inference API for MVP (free tier), migrate to self-hosted as scale increases.
...
result = emotion_classifier("I'm so excited about this!")
...
These ADRs represent the core architectural decisions that shape Delight's technology platform. Each decision balances:

- **Development velocity** (time to market)
- **Long-term maintainability** (avoiding technical debt)
- **Operational complexity** (fewer services to manage)
- **Cost efficiency** (staying within MVP budget)

<Info>
New architectural decisions should be documented as ADRs following this pattern. This helps future developers understand the "why" behind technical choices.
</Info>
...
<CardGroup cols={2}>
  <Card title="Technology Stack" href="/architecture/technology-stack" icon="cube">
    Explore the specific tools and libraries used in each layer.
  </Card>
  ...
  <Card title="API Contracts" href="/architecture/api-contracts" icon="plug">
    Review the REST API endpoints and data formats.
  </Card>
